...
//      proofread               — offline dictionary scan + optional LLM check
...
export function createHandler({ getConfig, updateConfig }) {
	return async function handler(endpoint, payload) {
		...
		if (endpoint === 'proofread') {
			const cfg = getConfig()
			const text = isPlainObject(payload) && typeof payload.text === 'string' ? payload.text.slice(0, 2000) : ''
			if (text.length === 0) return { ok: true, value: { issues: [] } }
			// Layer 1 — offline dictionary: always runs, instant, zero network.
			const local = cfg.proofreadEnabled === false ? [] : scanLocalTypos(text)
