...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			// —— 补/校开关：无边框幽灵样式，对齐宿主原生按钮观感 ——
			".ia_toggles{display:flex;align-items:center;gap:2px}",
			".ia_toggle{cursor:pointer;border:none;border-radius:6px;background:transparent;color:var(--dsw-alias-label-tertiary);font:inherit;font-size:12px;line-height:18px;padding:2px 6px;opacity:.65;transition:color .15s,opacity .15s}",
			".ia_toggle:hover{color:var(--dsw-alias-label-primary);opacity:1;background:var(--dsw-interactive-bg-hover)}",
			".ia_toggle.ia_on{color:var(--dsw-alias-brand-primary);opacity:1}",
		...
		const syncDockRows = () => {
			...
			const rows = document.querySelectorAll("[data-ia-dockrow]");
		...
		function TypoNav(props) {
			...
			return react.createElement("div", { className: "ia_nav", "data-input-assist": "proofread", "data-ia-dockrow": "" },
		...
		function GhostHint(props) {
			const { useInput, useAssist, useConfig, t } = props;
			if (typeof useAssist !== "function" || typeof useInput !== "function") return null;
			const assist = useAssist(identity);
			const cfg = useConfig(identity);
			const draft = useInput((s) => s.draft);
			...
			react.useEffect(() => { syncDockRows(); }, [visible]);
			if (!visible) return null;
			return react.createElement("div", {
				className: "ia_ghosthint",
				"data-input-assist": "ghosthint",
				"data-ia-dockrow": "",
			}, t("suggest.hint"));
		}
		...
		function apply(ctx) {
			...
			ctx.effect(() => locale.register(NS, {
				...
				en: {
					"toggle.completion": "Toggle input completion (when a suggestion shows: Tab accept word · Shift+Tab accept all · Esc close)",
			...
			const onViewportChange = () => { syncMirror(); syncDockRows(); };
			ctx.effect(() => {
